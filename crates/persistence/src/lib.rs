...
impl Persistence {
    ...
    pub async fn admin_list_restore_jobs(
        ...
        Ok(self
            .admin_list_restore_jobs_page(actor_user_id, project_id, 200, 0)
            .await?
            .0)
    }

    /// Lists one page of safe restore job metadata for operational monitoring.
    #[allow(clippy::missing_errors_doc)]
    pub async fn admin_list_restore_jobs_page(
        &self,
        actor_user_id: Uuid,
        project_id: Option<Uuid>,
        limit: i64,
        offset: i64,
    ) -> Result<(Vec<RestoreJobRecord>, bool), PersistenceError> {
        self.verify_admin(actor_user_id).await?;
        let mut tx = self.begin_admin().await?;
        let mut jobs = sqlx::query_as::<_, RestoreJobRecord>(
            "SELECT id, project_id, requested_by, snapshot_id, target_change_sequence, reason, \
                    status, pre_restore_snapshot_id, generation_before, generation_after, \
                    restored_objects, restored_tombstones, error_code, created_at, started_at, finished_at, \
                    attempts, run_after, cancel_requested \
        ...
        .fetch_all(&mut *tx)
        .await?;
        let has_more = i64::try_from(jobs.len()).unwrap_or(i64::MAX) > limit;
        if has_more {
            jobs.pop();
        }
        tx.commit().await?;
    ...
    /// Lists safe worker job metadata for operational dashboards.
    #[allow(clippy::missing_errors_doc)]
    pub async fn admin_list_jobs(
        ...
        Ok(self
            .admin_list_jobs_page(actor_user_id, kind, status, 500, 0)
            .await?
            .0)
    }

    /// Lists one page of safe worker job metadata for operational dashboards.
    #[allow(clippy::missing_errors_doc)]
    pub async fn admin_list_jobs_page(
        &self,
        actor_user_id: Uuid,
        kind: Option<&str>,
        status: Option<&str>,
        limit: i64,
        offset: i64,
    ) -> Result<(Vec<JobRecord>, bool), PersistenceError> {
        self.verify_admin(actor_user_id).await?;
        let mut tx = self.begin_admin().await?;
        let mut jobs = sqlx::query_as::<_, JobRecord>(
            "SELECT id, kind, owner_user_id, project_id, status, attempts, run_after, \
                    error_code, created_at, started_at, finished_at \
    ...
    pub async fn admin_list_sync_attempts(
        ...
        Ok(self
            .admin_list_sync_attempts_page(actor_user_id, 200, 0)
            .await?
            .0)
    }

    /// Lists one page of synchronization diagnostics without payload data.
    #[allow(clippy::missing_errors_doc)]
    pub async fn admin_list_sync_attempts_page(
        &self,
        actor_user_id: Uuid,
        limit: i64,
        offset: i64,
    ) -> Result<(Vec<SyncAttemptRecord>, bool), PersistenceError> {
        self.verify_admin(actor_user_id).await?;
        let mut tx = self.begin_admin().await?;
        let mut attempts = sqlx::query_as::<_, SyncAttemptRecord>(
        ...
        .fetch_all(&mut *tx)
        .await?;
        let has_more = i64::try_from(attempts.len()).unwrap_or(i64::MAX) > limit;
        if has_more {
            attempts.pop();
        }
        tx.commit().await?;
